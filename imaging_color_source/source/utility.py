...
def clean_to_images_and_save(clean_folder, clean_name, nterm, nu_arr, nu0):
	...
	hdul.close()
	images_taylor = np.ones((nterm, nx, ny))
	...
	## residual convert
	fileimage_residual = "./%s/%s.residual.tt0.fits"  % (clean_folder, clean_name)
	hdul = fits.open(fileimage_residual)
	image_residual = np.fliplr(np.array(hdul[0].data[0][0])/beam_div_pixel2)
	hdul.close()
	...
	for i in range(nterm-1):
		...
		image_taylor_now = image_taylor_now/beam_div_pixel2
		hdul.close()
	...
	np.savez("./%s/I0_alpha_clean" % clean_folder, I0 = image_taylor0, alpha = image_alpha, \
		image_nu = images_freqs, residual = image_residual)
